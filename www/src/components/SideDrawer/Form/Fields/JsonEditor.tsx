...
import { makeStyles, createStyles, useTheme, withStyles } from "@material-ui/core";
//import { TextField } from "formik-material-ui";
import TextField from '@material-ui/core/TextField';
import { valueCellContentRenderer } from "react-data-grid";
...
const CustomRow = ({val, field, form, shareVal, index}) => {
        ...
        const finalEdit = (name) => {
          setisEditing(false);
          if(!isNaN(shares)){
            //console.log("Input: " + e.target.value + " @ " + name);
            field.value[name].shareTotal = shares;
        ...
        <Table className={classes.table} aria-label="simple table">
          ...
          <TableBody>
            {Object.entries(field.value).map((val: any, index) => {
              return(
              <CustomRow field={field} form={form} val={val} index={index} shareVal={val[1].shareTotal / Math.pow(10, 8)}/>
              )
            })}
          </TableBody>
        </Table>
